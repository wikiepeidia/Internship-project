...
| `scripts/measure_cold_latency.py` | `historical` | `python` | `latency_probe` | `phase_30` | `__future__`<br>`argparse`<br>`datetime`<br>`json`<br>`pathlib`<br>`playwright.sync_api`<br>`subprocess`<br>`sys`<br>`time`<br>`traceback`<br>`typing` | `powercfg /GETACTIVESCHEME`<br>`powershell -NoProfile -Command (Get-CimInstance Win32_OperatingSystem).LastBootUpTime.ToString('o')`<br>`python -m src.runtime.cli demo --no-browser --port {port}` |
...
| `scripts/vastai_gguf_export.sh` | `historical` | `bash` | `model_export_workflow` | `phase_neutral` | `json`<br>`os`<br>`pathlib`<br>`peft`<br>`torch`<br>`transformers`<br>`transformers.generation` | — |
| `scripts/vastai_qlora_full.sh` | `historical` | `bash` | `training_workflow` | `phase_neutral` | `collections`<br>`huggingface_hub`<br>`json`<br>`pathlib`<br>`peft`<br>`re`<br>`torch`<br>`transformers` | `python3 -m src.model_adaptation.cli train --candidate baseline-winner --version-tag {version} --train-split /workspace/train.jsonl --val-split /workspace/val.jsonl --output-root {model_root} --registry-path {registry} --device cuda`<br>`bash {repo}/scripts/vastai_gguf_export.sh` |
| `scripts/verify_golden_prompts.py` | `historical` | `python` | `verification_runner` | `phase_28` | `__future__`<br>`argparse`<br>`datetime`<br>`json`<br>`pathlib`<br>`playwright.sync_api`<br>`subprocess`<br>`sys`<br>`time`<br>`traceback`<br>`typing` | `vnphish demo --no-browser --port {port}` |
| `scripts/verify_phase32_fresh_process.py` | `historical` | `python` | `verification_runner` | `phase_32` | `__future__`<br>`argparse`<br>`datetime`<br>`importlib.util`<br>`json`<br>`os`<br>`pathlib`<br>`playwright.sync_api`<br>`re`<br>`subprocess`<br>`sys`<br>`time`<br>`traceback`<br>`typing` | `netstat -ano -p tcp`<br>`taskkill /PID {pid} /T /F`<br>`cmd.exe /c {repo}/scripts/START_DEMO_UI.bat`<br>`{repo}/scripts/START_DEMO_UI.bat`<br>`taskkill /PID {process.pid} /T /F` |
| `scripts/verify_ui_quirks.py` | `historical` | `python` | `verification_runner` | `phase_31` | `__future__`<br>`argparse`<br>`datetime`<br>`json`<br>`pathlib`<br>`playwright.sync_api`<br>`subprocess`<br>`sys`<br>`time`<br>`traceback`<br>`typing` | `python -m src.runtime.cli demo --no-browser --port {port}` |
