...
| `scripts/measure_cold_latency.py` | `historical` | `python` | `latency_probe` | `phase_30` | `__future__`<br>`argparse`<br>`datetime`<br>`json`<br>`pathlib`<br>`playwright.sync_api`<br>`subprocess`<br>`sys`<br>`time`<br>`traceback`<br>`typing` | `python -m src.runtime.cli demo --no-browser --port {port}` |
...
| `scripts/verify_phase32_fresh_process.py` | `historical` | `python` | `verification_runner` | `phase_32` | `__future__`<br>`argparse`<br>`datetime`<br>`importlib.util`<br>`json`<br>`os`<br>`pathlib`<br>`playwright.sync_api`<br>`re`<br>`subprocess`<br>`sys`<br>`time`<br>`traceback`<br>`typing` | `cmd.exe /c {repo}/scripts/START_DEMO_UI.bat`<br>`{repo}/scripts/START_DEMO_UI.bat` |
