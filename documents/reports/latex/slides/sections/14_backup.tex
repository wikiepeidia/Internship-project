...
\begin{frame}[allowframebreaks]{Backup: Evaluation \& Confusion --- Full Detail}
  ...
  \begin{itemize}\footnotesize
    ...
    \item \textbf{Caution:} these model results belong to the older 254-row trainer-validation snapshot; they were not recomputed on the promoted 2{,}097-row corpus.
  \end{itemize}
  \vspace{4pt}
...
\begin{frame}[allowframebreaks]{Backup: Contributions \& Future --- Full Detail}
  \begin{block}{Contributions (original, pre-merge)}
    {\footnotesize
    \begin{enumerate}
      \item \textbf{Vietnamese phishing corpus:} 2{,}097 promoted records, 4 classes, with schema validation and lineage governance.
      \item \textbf{Privacy-first local runtime:} \texttt{Qwen3-4B-Instruct-2507} GGUF Q8\_0 via \texttt{llama.cpp}; ${\approx}24$\,s in the final laptop CLI verification; no cloud inference.
      \item \textbf{Explainable decision layer:} grounded cue extraction + risk-tier labels in Vietnamese.
      \item \textbf{Historical internal model evaluation:} 254 validation samples; macro F1 = 0.9625; no current-corpus or independent-test claim.
    \end{enumerate}}
  \end{block}
  ...
  \begin{itemize}\footnotesize
    ...
    \item The 254-row result is an older model-training snapshot and was not recomputed on the promoted final corpus.
  \end{itemize}
  \begin{block}{Primary Future Direction (original, pre-merge)}
    {\footnotesize Train and select without touching the promoted 220-row test split, evaluate it once, and add real-world labelled evidence.}
  \end{block}
\end{frame}
