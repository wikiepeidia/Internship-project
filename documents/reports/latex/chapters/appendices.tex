...
An earlier version of this work used a different 254-message validation set, shown in Table~\ref{tab:validation-class-dist} for historical interest only. Its numbers were never recalculated on the final dataset. The only place it appears in this report is the prompting-against-fine-tuning check in Section~\ref{sec:why-finetune}, which is reported there as the reason for fine-tuning at all. None of the model results reported in Chapter~V come from it.
