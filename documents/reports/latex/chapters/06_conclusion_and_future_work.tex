...
Two models were trained locally on the same messages, with one seed. Qwen was trained with 4-bit QLoRA and PhoBERT as a full four-class classifier. On the 220 held-out messages, PhoBERT scored higher overall, at macro F1 0.990892 against Qwen's 0.980493. Neither model produced an unreadable answer, and each made exactly one dangerous mistake, calling a scam safe.

The last point is the one that matters most. On the measure that matters most for a phishing tool, the two models tied. And since each was trained only once, the overall gap describes this run rather than proving that PhoBERT is reliably better.
...
Two pieces of work would matter most next.
