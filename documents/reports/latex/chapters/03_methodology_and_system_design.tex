...
\subsection*{Sources of the Messages}
...
\noindent\textbf{Record fields:} Table~\ref{tab:record-schema} lists the seven fields every message carries. Four of them are the fields described in Chapter~I that the model is trained to produce; the other three, \texttt{text}, \texttt{source}, and \texttt{seed\_id}, are the input itself and the provenance information used to build and split the dataset, not something the model learns. The \texttt{label} field is the one the model is graded on, it is present on every message, and it is the training target for both models.
...
\subsection*{Acceptance of Label Corrections}
...
\subsection{Fine-Tuning Against Prompting}
\label{sec:why-finetune}

Choosing a model does not answer a more basic question: whether the model needed to be trained at all, or whether a careful prompt would have been enough. This project measured that instead of assuming it, by scoring the chosen model on the same 254 messages twice, once with prompting alone and once after QLoRA. Table~\ref{tab:finetuning-gain} gives both.
...
PhoBERT has no equivalent stock-versus-trained check, and the reason is structural rather than an oversight. Qwen is a general-purpose model that can attempt a task from a prompt alone, so prompting it with no fine-tuning is a real, measurable thing to try. PhoBERT is an encoder with a four-class head added on top, and an untrained head only outputs random weights; there is nothing meaningful to prompt or to measure before that head is trained. So Chapter~V compares two \emph{trained} models against each other, and this section separately checks, for Qwen only, whether training was worth doing in the first place.
...
\section{How the Runtime Serves an Answer}

The runtime has one text-only interface. It accepts a message as text, cleans it up locally, and passes it to whichever backend is configured. The backend returns a structured result, and a rendering layer turns that into a readable report.
...
\subsection*{Handling Free-Text Output}
