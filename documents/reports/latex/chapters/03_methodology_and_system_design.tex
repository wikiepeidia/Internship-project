...
The five stages listed in Chapter~I have to run in that order, and the order is the part worth explaining. The dataset has to exist and be split before anything can be trained on it. The hardware has to be measured before a training method is chosen, because otherwise the choice is a guess. The runtime has to be built around whatever the model actually produces rather than around what it was hoped to produce. Finally, the held-out messages cannot be touched until both models are finished, because a set that has already been consulted is no longer held out. Each of the following sections explains one of those decisions.
...
\noindent A scoring layer would still be the better choice if a label were all that was needed, but this system also has to produce the risk tier, the quoted evidence, and the recommendation.
...
Two rules keep those explanations honest. Quoted phrases must come from the message text itself, and recommendations must stay inside a safe, generic set that cannot tell a user to do something harmful.
