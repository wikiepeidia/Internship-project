...
Keeping those four questions apart matters, because a high validation score does not prove the dataset is realistic, a hardware measurement says nothing about accuracy, and a single run on held-out data cannot show how much the number would move if everything were run again.
...
The judge model scored every message. Of those, 1{,}561 kept a score from an earlier round because the message had not changed since, and 536 were scored fresh. In total 1{,}395 passed, which is 66.52\%. A message passes only if it scores at least 3 out of 5 on all five things the judge looks at. Figure~\ref{fig:corpus-quality} gives the average for each of the five and how many messages passed in each class.
...
Two things in that figure need saying before anyone asks. The two lowest averages are for whether a message reads like a real scam and whether its risk level fits, which are the two hardest things to get right when a model is writing the message rather than a person. The Zalo class also passes at 99.7\% while task scam passes at 47.5\%. That gap is not evidence that the Zalo messages are better. They are the ones that were rewritten, and they were rewritten by a model from the same family as the judge that then scored them. It is the clearest example in this report of why the judge is not treated as a second opinion.
...
\item \textbf{The judge is not fully independent.} The rewritten Zalo messages were written by a model from the same family as the judge that scored them, and they pass at 99.7\% against 47.5\% for task scam. For those messages the judge is checking work related to its own, so its verdict is not a second opinion.
