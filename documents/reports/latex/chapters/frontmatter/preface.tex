...
This thesis presents a pipeline for fine-tuning a local large language model (LLM) to detect Vietnamese financial phishing messages while preserving user privacy through on-device inference. A labelled dataset of 3,000 Vietnamese messages is constructed from cybercrime alert seeds, synthetic generation, Pydantic schema validation, and an LLM-based sample review. Qwen3-4B-Instruct-2507 is adapted using quantized low-rank adaptation (QLoRA) and deployed as a GGUF Q8\_0 artifact for CPU-based laptop inference via llama.cpp. A structured decision layer returns a risk tier, threat label, grounded evidence cues, and safety recommendations. On a 254-example internal validation split, the adapted model yields macro F1 0.9625, compared with 0.6702 for the same base model without adaptation. This result is limited by validation reuse and seed overlap across splits; it is not an independent test estimate. Deployment in safety-critical settings is not recommended without evaluation on a real-world labelled corpus.
