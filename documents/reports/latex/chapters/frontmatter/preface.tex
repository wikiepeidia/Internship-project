...
We present a pipeline for fine-tuning a local large language model (LLM) to detect Vietnamese financial phishing messages while preserving user privacy through on-device inference. A reproducible labelled dataset of 3,000 Vietnamese messages is constructed from cybercrime alert seeds combined with synthetic generation and quality judging. A Qwen3.5-4B model is adapted using quantized low-rank adaptation (QLoRA) and deployed as a GGUF Q8\_0 artifact for CPU-based laptop inference via llama.cpp. A structured decision layer returns a risk tier, threat label, grounded evidence cues, and actionable safety recommendations for each input. Evaluation on a 254-example seed-disjoint internal holdout yields a macro F1 of 0.9553; all three risky classes meet their per-class recall targets, with task-scam recall reaching 0.871 on 62 held-out examples. A binary scam-versus-non-scam evaluation over the same holdout achieves a binary F1 of 1.000. Deployment in safety-critical settings is not recommended without validation on a real-world labelled corpus.
