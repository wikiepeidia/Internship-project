...
- Phase 41 complete and closed (2026-08-26): the finalized Qwen QLoRA and PhoBERT checkpoints received one shared-cohort model-evaluation pass, the results and selected identities were frozen, and the verified export plus provenance erratum now form the inseparable reporting authority. Held-out results are terminal evidence and did not trigger tuning, repair, or retry.
...
- [x] Run the current reserved 220-row test split exactly once, at the end, across Qwen QLoRA and PhoBERT; results are frozen and did not trigger retraining, dataset repair, or checkpoint selection.
- [ ] Complete Phase 41.1's compatibility-first architecture overhaul: preserve the exact historical producer closure, split the model-adaptation CLI, create phase-neutral active boundaries, retain old paths as shims, and publish the report-ready provenance/storage handoff.
...
| Insert Phase 41.1 before the report rewrite as a compatibility-first architecture overhaul | The active repository is dominated by chronology-named experiment tooling and an oversized operator CLI. A six-wave strangler migration makes the current system explainable without rewriting the exact source/evidence that produced Phase 40/41 results. | Accepted 2026-08-27 |
...
- Preserve the completed one-pass 220-row two-model evaluation as terminal evidence. Its verified export and provenance erratum disclose prior human/content exposure and the exact access limitation; no held-out result triggered tuning, repair, thresholding, or model selection. Any optional all-data deployment fit remains separately labeled and receives no unbiased test-score claim.
- Before rewriting the report, complete the Phase 41.1 compatibility-first architecture overhaul: archive the exact evaluated source closure from its manifest-matching ProgramData authority, preserve all public/legacy CLI contracts, separate active runtime/modeling/data responsibilities, and document historical provenance plus D-drive retention without deleting artifacts.
...
**Timeline:** Phase 40 training and Phase 41 terminal evaluation are complete. Execute the six Phase 41.1 architecture waves next, then use their report-ready handoff plus the frozen evaluation authority for Phase 42/43; finish the student-authored Phase 44 walkthrough immediately before the retake defense.
...
- v7.0 active (2026-08-27): Phases 38–41 are complete and their corpus/training/evaluation evidence is frozen. Phase 41.1 is planned in six independently checked waves and is next to execute; it changes architecture only, never the models, dataset, held-out metrics, or historical evidence authorities. Phase 42 now depends on its report-ready architecture/provenance handoff.
