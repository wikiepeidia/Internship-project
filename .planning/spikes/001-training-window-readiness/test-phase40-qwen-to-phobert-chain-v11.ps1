...
$v11Controller = Join-Path $PSScriptRoot 'phase40-qwen-to-phobert-chain-v11.ps1'
$expectedV9Sha256 = '63f47598fe81749b961ca7c5f056fe4e63925f2ad93f94f9beabafd047246b26'
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) (
    'phase40-controller-v11-' + [Guid]::NewGuid().ToString('N')
)
$loadedModule = $null
...
try {
    ...
    $loadedModule = Import-SyntheticTrainingModule `
        -ControllerPath $v11Controller `
        -Label 'v11'
    $freshExit = Invoke-SyntheticFreshTraining
    Assert-True ($freshExit -eq 0) 'v11 fresh launch did not reach the synthetic launcher'
    ...
    Assert-True ($freshInvocation.Name -eq 'phobert-train-fresh') `
        'v11 fresh operation name changed'
    Assert-True ($freshArguments -notcontains '--resume-from-checkpoint') `
        'v11 fresh arguments contain a resume flag'
    Assert-True ($freshArguments -contains '..\phobert-work-v11\phase40-phobert-full-seed42-v11') `
        'v11 fresh arguments do not target the v11 work root'
    Assert-True ($freshArguments -notcontains '..\phobert-work-v10\phase40-phobert-full-seed42-v1') `
        'v11 fresh arguments still target the failed v10 work root'
    Assert-True ($freshArguments -contains '..\transfer-root-v4\data\models\phase40\full-run-request.json') `
        'v11 fresh arguments do not use the v4 request'
    Assert-True ($freshArguments -contains '..\transfer-root-v4\data\models\phase40\input\phase40-train-validation.zip') `
        'v11 fresh arguments do not use the v4 input authority'
    Assert-True ($freshArguments -contains '..\transfer-root-v3\data\models\phase40\base\phobert-base-v2') `
        'v11 fresh arguments do not reuse the verified v3 base model'
    ...
    Assert-True ($resumeExit -eq 0) 'v11 resume launch did not reach the synthetic launcher'
    ...
    Assert-True ($resumeInvocation.Name -eq 'synthetic-resume') `
        'v11 resume operation name changed'
    Assert-True ($resumeFlagIndex -ge 0 -and
        $resumeFlagIndex + 1 -lt $resumeArguments.Count) `
        'v11 resume arguments omit the checkpoint value'
    Assert-True ($resumeArguments[$resumeFlagIndex + 1] -eq
        [System.IO.Path]::GetFullPath($checkpoint)) `
        'v11 resume checkpoint is not canonicalized'

    Write-Output 'PASS phase40 controller v11 CPU-only launch scenarios=3'
}
finally {
