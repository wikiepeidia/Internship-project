...
$workParent = Join-Path $packageRoot 'phobert-work-v10'
...
$telemetryPath = Join-Path $controllerRoot 'system-telemetry-phobert-v10.csv'
$telemetryStdout = Join-Path $controllerRoot 'telemetry-phobert-v10.stdout.log'
$telemetryStderr = Join-Path $controllerRoot 'telemetry-phobert-v10.stderr.log'
$telemetryStopPath = Join-Path $controllerRoot 'telemetry-phobert-v10.stop.json'
$summaryPath = Join-Path $controllerRoot 'system-telemetry-summary-phobert-v10.json'
$telemetryVerificationReceiptPath = Join-Path $controllerRoot 'system-telemetry-summary-phobert-v10.verification.json'
$chainLog = Join-Path $controllerRoot 'qwen-to-phobert-chain-v10.log'
$runtimePreflightPath = Join-Path $controllerRoot 'source-runtime-preflight-v10.json'
...
$pythonCacheRoot = Join-Path $packageRoot ("python-cache-v10-{0}" -f $pythonCacheInstanceId)
...
function Assert-FrozenSourceRuntime {
    ...
    if ($Stage -eq 'python-launch') {
        ...
        $stagePath = Join-Path $controllerRoot (
            'source-runtime-preflight-v10-python-launch-{0}-{1}-{2:D3}-{3}.json' -f
                $controllerPid,
                $controllerCreationUtcFileTimeTicks,
        ...
        $stagePath = Join-Path $controllerRoot "source-runtime-preflight-v10-$Stage.json"
    }
    $payload = [ordered]@{
...
function Invoke-PythonCaptured {
    ...
    $stdoutPath = Join-Path $controllerRoot "$Name.phobert-v10.stdout.log"
    $stderrPath = Join-Path $controllerRoot "$Name.phobert-v10.stderr.log"
...
function Invoke-PhoBertTraining {
    ...
    foreach ($argument in @(
        ...
        '--run-id', $runId,
        '--output-root', '..\phobert-work-v10\phase40-phobert-full-seed42-v1',
        '--base-model-path', '..\transfer-root-v3\data\models\phase40\base\phobert-base-v2',
        '--base-model-manifest-path', '..\transfer-root-v3\data\models\phase40\base\phobert-base-v2.provenance.json'
    )) {
