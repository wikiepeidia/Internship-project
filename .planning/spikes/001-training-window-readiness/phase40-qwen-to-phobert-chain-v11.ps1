...
$sourceRuntime = Join-Path $packageRoot 'source-runtime-v11'
$transferRoot = Join-Path $packageRoot 'transfer-root-v4'
$completedTransferRoot = Join-Path $packageRoot 'transfer-root-v3'
...
$baseModelPath = Join-Path $completedTransferRoot 'data\models\phase40\base\phobert-base-v2'
$baseManifestPath = Join-Path $completedTransferRoot 'data\models\phase40\base\phobert-base-v2.provenance.json'
$workParent = Join-Path $packageRoot 'phobert-work-v11'
$workRoot = Join-Path $workParent 'phase40-phobert-full-seed42-v11'
$trainerRoot = Join-Path $workRoot 'trainer'
$runRoot = Join-Path $transferRoot 'data\models\phase40\full\phobert'
$qwenRunRoot = Join-Path $completedTransferRoot 'data\models\phase40\full\qwen-qlora'
...
$telemetryPath = Join-Path $controllerRoot 'system-telemetry-phobert-v11.csv'
$telemetryStdout = Join-Path $controllerRoot 'telemetry-phobert-v11.stdout.log'
$telemetryStderr = Join-Path $controllerRoot 'telemetry-phobert-v11.stderr.log'
$telemetryStopPath = Join-Path $controllerRoot 'telemetry-phobert-v11.stop.json'
$summaryPath = Join-Path $controllerRoot 'system-telemetry-summary-phobert-v11.json'
$telemetryVerificationReceiptPath = Join-Path $controllerRoot 'system-telemetry-summary-phobert-v11.verification.json'
$chainLog = Join-Path $controllerRoot 'qwen-to-phobert-chain-v11.log'
$runtimePreflightPath = Join-Path $controllerRoot 'source-runtime-preflight-v11.json'
...
$pythonCacheRoot = Join-Path $packageRoot ("python-cache-v11-{0}" -f $pythonCacheInstanceId)
$pythonExe = 'C:\Users\wikiepeidia\AppData\Local\Programs\Python\Python313\python.exe'
$pwshExe = Join-Path $PSHOME 'pwsh.exe'
$runId = 'phase40-phobert-full-seed42-v11'
$expectedRequestSha256 = '1521917ac155a06fd9ff1ff9918e45bd73a5e189af692a69b1a9b88580e20592'
$expectedSourceArchiveSha256 = '0e76ee9fda535ff83e48de11a08877ba44e1ba112e52a42d54718793e67c64ac'
$expectedSourceManifestSha256 = 'f6181c8fa074f1c49bd2984c08e4a4163ff5095a6410268bd43e805ec82cebdf'
...
function Assert-FrozenSourceRuntime {
    ...
    if ($Stage -eq 'python-launch') {
        ...
        $stagePath = Join-Path $controllerRoot (
            'source-runtime-preflight-v11-python-launch-{0}-{1}-{2:D3}-{3}.json' -f
                $controllerPid,
                $controllerCreationUtcFileTimeTicks,
        ...
        $stagePath = Join-Path $controllerRoot "source-runtime-preflight-v11-$Stage.json"
    }
    $payload = [ordered]@{
...
function Invoke-PythonCaptured {
    ...
    $stdoutPath = Join-Path $controllerRoot "$Name.phobert-v11.stdout.log"
    $stderrPath = Join-Path $controllerRoot "$Name.phobert-v11.stderr.log"
...
function Invoke-PhoBertDoctor {
    return Invoke-PythonCaptured -Name 'phobert-doctor' -Arguments @(
        ...
        '--run-request-path', '..\transfer-root-v4\data\models\phase40\full-run-request.json',
        '--repo-root', '..\transfer-root-v4',
...
function Invoke-PhoBertTraining {
    ...
    foreach ($argument in @(
        ...
        '--request-path', '..\transfer-root-v4\data\models\phase40\full-run-request.json',
        '--repo-root', '..\transfer-root-v4',
        '--input-archive', '..\transfer-root-v4\data\models\phase40\input\phase40-train-validation.zip',
        '--extraction-root', '/content/phase40-input-v1',
        '--run-id', $runId,
        '--output-root', '..\phobert-work-v11\phase40-phobert-full-seed42-v11',
        '--base-model-path', '..\transfer-root-v3\data\models\phase40\base\phobert-base-v2',
        '--base-model-manifest-path', '..\transfer-root-v3\data\models\phase40\base\phobert-base-v2.provenance.json'
    )) {
        [void]$arguments.Add($argument)
    }
    if (-not [string]::IsNullOrEmpty($ResumeFromCheckpoint)) {
