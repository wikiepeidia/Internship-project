...
$v10Controller = Join-Path $PSScriptRoot 'phase40-qwen-to-phobert-chain-v10.ps1'
$expectedV9Sha256 = '63f47598fe81749b961ca7c5f056fe4e63925f2ad93f94f9beabafd047246b26'
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) (
    'phase40-controller-v10-' + [Guid]::NewGuid().ToString('N')
)
$loadedModule = $null
...
try {
    ...
    $loadedModule = Import-SyntheticTrainingModule `
        -ControllerPath $v10Controller `
        -Label 'v10'
    $freshExit = Invoke-SyntheticFreshTraining
    Assert-True ($freshExit -eq 0) 'v10 fresh launch did not reach the synthetic launcher'
    ...
    Assert-True ($freshInvocation.Name -eq 'phobert-train-fresh') `
        'v10 fresh operation name changed'
    Assert-True ($freshArguments -notcontains '--resume-from-checkpoint') `
        'v10 fresh arguments contain a resume flag'
    Assert-True ($freshArguments -contains '..\phobert-work-v10\phase40-phobert-full-seed42-v1') `
        'v10 fresh arguments do not target the v10 work root'
    Assert-True ($freshArguments -notcontains '..\phobert-work-v9\phase40-phobert-full-seed42-v1') `
        'v10 fresh arguments still target the failed v9 work root'
    ...
    Assert-True ($resumeExit -eq 0) 'v10 resume launch did not reach the synthetic launcher'
    ...
    Assert-True ($resumeInvocation.Name -eq 'synthetic-resume') `
        'v10 resume operation name changed'
    Assert-True ($resumeFlagIndex -ge 0 -and
        $resumeFlagIndex + 1 -lt $resumeArguments.Count) `
        'v10 resume arguments omit the checkpoint value'
    Assert-True ($resumeArguments[$resumeFlagIndex + 1] -eq
        [System.IO.Path]::GetFullPath($checkpoint)) `
        'v10 resume checkpoint is not canonicalized'

    Write-Output 'PASS phase40 controller v10 CPU-only launch scenarios=3'
}
finally {
