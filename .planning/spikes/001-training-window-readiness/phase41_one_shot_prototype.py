"""Standard-library spike for a fail-closed, one-shot Phase 41 evaluation.

This prototype deliberately has no CLI and no model-library imports.  The
caller must inject the sole reserved-split opener and the two already-frozen
model predictors.  The durable claim is created before that opener is called;
once the claim exists the evaluation is spent, even when a later operation
fails.

The injected callback counts are contract tests, not proof of an OS-level
single-open or of model identity. A production executor must own the file
handle and construct both adapters from independently verified model evidence.
"""
...
def prepare_evaluation(
    ...
    *,
    ...
    split = SplitIdentity(
        path=os.path.abspath(os.path.normpath(os.fspath(reserved_split_path))),
...
def _global_claim_path(identity: SplitIdentity) -> Path:
    ...
    return _CANONICAL_CLAIM_REGISTRY_ROOT / f"{identity.sha256}.claim.json"
