...
$sourceRuntime = Join-Path $packageRoot 'source-runtime-v12'
$transferRoot = Join-Path $packageRoot 'transfer-root-v5'
...
$workParent = Join-Path $packageRoot 'phobert-work-v12'
$workRoot = Join-Path $workParent 'phase40-phobert-full-seed42-v12'
...
$telemetryPath = Join-Path $controllerRoot 'system-telemetry-phobert-v12.csv'
$telemetryStdout = Join-Path $controllerRoot 'telemetry-phobert-v12.stdout.log'
$telemetryStderr = Join-Path $controllerRoot 'telemetry-phobert-v12.stderr.log'
$telemetryStopPath = Join-Path $controllerRoot 'telemetry-phobert-v12.stop.json'
$summaryPath = Join-Path $controllerRoot 'system-telemetry-summary-phobert-v12.json'
$telemetryVerificationReceiptPath = Join-Path $controllerRoot 'system-telemetry-summary-phobert-v12.verification.json'
$chainLog = Join-Path $controllerRoot 'qwen-to-phobert-chain-v12.log'
$runtimePreflightPath = Join-Path $controllerRoot 'source-runtime-preflight-v12.json'
...
$pythonCacheRoot = Join-Path $packageRoot ("python-cache-v12-{0}" -f $pythonCacheInstanceId)
$pythonExe = 'C:\Users\wikiepeidia\AppData\Local\Programs\Python\Python313\python.exe'
$pwshExe = Join-Path $PSHOME 'pwsh.exe'
$runId = 'phase40-phobert-full-seed42-v12'
$expectedRequestSha256 = 'f95f20a4094179eda62d7f255ae426d3971bc214fed19f93d30173d5fb9553e2'
...
function Assert-FrozenSourceRuntime {
    ...
    if ($Stage -eq 'python-launch') {
        ...
        $stagePath = Join-Path $controllerRoot (
            'source-runtime-preflight-v12-python-launch-{0}-{1}-{2:D3}-{3}.json' -f
                $controllerPid,
                $controllerCreationUtcFileTimeTicks,
        ...
        $stagePath = Join-Path $controllerRoot "source-runtime-preflight-v12-$Stage.json"
    }
    $payload = [ordered]@{
...
function Invoke-PythonCaptured {
    ...
    $stdoutPath = Join-Path $controllerRoot "$Name.phobert-v12.stdout.log"
    $stderrPath = Join-Path $controllerRoot "$Name.phobert-v12.stderr.log"
...
function Invoke-PhoBertDoctor {
    return Invoke-PythonCaptured -Name 'phobert-doctor' -Arguments @(
        ...
        '--run-request-path', '..\transfer-root-v5\data\models\phase40\full-run-request.json',
        '--repo-root', '..\transfer-root-v5',
        '--input-root', 'D:\content\phase40-input-v1',
        '--base-model-path', '..\transfer-root-v5\data\models\phase40\base\phobert-base-v2',
        '--base-model-manifest-path', '..\transfer-root-v5\data\models\phase40\base\phobert-base-v2.provenance.json'
    )
}
...
function Invoke-PhoBertTraining {
    ...
    foreach ($argument in @(
        ...
        '--request-path', '..\transfer-root-v5\data\models\phase40\full-run-request.json',
        '--repo-root', '..\transfer-root-v5',
        '--input-archive', '..\transfer-root-v5\data\models\phase40\input\phase40-train-validation.zip',
        '--extraction-root', '/content/phase40-input-v1',
        '--run-id', $runId,
        '--output-root', '..\phobert-work-v12\phase40-phobert-full-seed42-v12',
        '--base-model-path', '..\transfer-root-v5\data\models\phase40\base\phobert-base-v2',
        '--base-model-manifest-path', '..\transfer-root-v5\data\models\phase40\base\phobert-base-v2.provenance.json'
    )) {
