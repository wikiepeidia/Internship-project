...
- [x] 41.1-03-PLAN.md -- Extract integrity, artifact, settings, runtime-import, and dependency-extra boundaries

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 41.1-04-PLAN.md -- Introduce phase-neutral modeling services, read-only evidence consumption, and one-way legacy adapters

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 41.1-05-PLAN.md -- Separate reusable data core, external workflows, and traceable one-off migrations

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 41.1-06-PLAN.md -- Enforce architecture policy and publish report-ready provenance/storage handoff
...
**Gating note (superseded 2026-08-31 — decision, not a placeholder guess):** The reference report never arrived and the student decided not to keep waiting for it, given the ~2-week deadline: "we mostly failed because of credibility [in the defense Q&A], as long as that's not cooked, report issue won't matter — bachelor thesis issues happen all the time." REPORT-03 (deriving `WRITING_GUARDRAILS_REPORT.md` from a real reference) is deprioritized, not fabricated against a guess. Chapter rewrite proceeds directly against the existing `documents/reports/latex/WRITING_GUARDRAILS.md` (Phase 8's general style guide — still real, just not reference-derived) using the actual mechanism REPORT-04 requires either way: the student hand-drafts each chapter passage, Claude tightens grammar/tone only, never generates or restructures voice. Defense Q&A readiness (`documents/defense/DEFENSE_QA_WORKSHEET.md`, the defense-coach protocol in CLAUDE.md) is the higher current priority per this same decision.
...
| 41.1 Codebase Architecture Overhaul | 15/15 | In Progress|  |
